  ...
  ({ error, onDismiss }: { error: string; onDismiss: () => void }) => (
    <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-50 animate-fade-in animate-slide-up">
      <div className="bg-red-100 dark:bg-red-900/50 border border-red-400 dark:border-red-700 text-red-700 dark:text-red-300 px-6 py-3 rounded-lg shadow-md max-w-md flex items-center">
        ...
        <button
          onClick={onDismiss}
          className="ml-auto text-red-700 dark:text-red-300 hover:text-red-800 dark:hover:text-red-200"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-4 w-4"
...
function ChatWindow() {
  ...
  return (
    <div className="flex-1 flex flex-col relative">
      ...
      {/* Loading indicator overlay - only show for new messages, not initial load */}
      {loading && (
        <div className="fixed bottom-24 right-8 bg-white dark:bg-gray-800 rounded-full p-3 shadow-lg z-20">
          <svg
